...
exports.getYearReference = async (req, res, next) => {
    const doc = await Reference.find({ model: req.params.model, field: req.params.field, year: req.params.year });
...
exports.updateYearReference = async (req, res, next) => {
    doc = await Reference.getNewReference(req.params.model, req.params.field, req.params.year);
    res.status(200).json({
        status: 'success',
        data: {
            data: doc,
